...
## Site-Specific Typeballs (Digital Twin)

The following Typeball packages have been generated using the `oclaw site map build` and `oclaw cli gen` pipeline per **WSP 103**. These serve as the standardized capability endpoints mapped by UI-TARS/DOM inspection for specific target sites.

| Site Slug      | Path                     | Primary Engine | Capabilities        |
| -------------- | ------------------------ | -------------- | ------------------- |
| _Example Site_ | `typeballs/examplesite/` | Playwright     | Login, Post Message |

_To map a new site, execute: `oclaw site scan --url <target>`_
...
python holo_index.py --search "agent typeball registry mapping"
```

**Tags for indexing**: `#agent-cli`, `#openclaw`, `#ironclaw`, `#json-output`, `#automation`, `#typeball`, `#digital-twin`

---

_WSP 77 / WSP 103: Agentic Navigation & CLI Interface Coordination_
