...
import { WREIntegration } from './wre_integration';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const wreIntegration = new WREIntegration(context);
            ...
            try {
                ...
                if (result.success) {
                    ...
                    vscode.window.showInformationMessage(
                        `WRE Session Started: ${result.sessionId}`,
                        'View Logs', 'WRE Status'
                    ).then(selection => {
                    ...
                    vscode.window.showErrorMessage(
                        `WRE Failed to Start: ${result.error}`,
                        'View Logs', 'Retry'
                    ).then(selection => {
                        if (selection === 'View Logs') {
                            wreIntegration.showLogs();
