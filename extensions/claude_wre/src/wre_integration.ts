...
import { AgentDashboard, AgentActivity, FileChange } from './agent_dashboard';
...
interface WREStatus {
    ...
    wspCoreLoaded: boolean;
    agentSuiteStatus: string;
    complianceScore: number;
    communicationActive: boolean;
    contextSync: boolean;
}

export class WREIntegration implements vscode.Disposable {
    private context: vscode.ExtensionContext;
    private wreProcess: ChildProcess | null = null;
    private websocket: WebSocket | null = null;
    private outputChannel: vscode.OutputChannel;
    private isActive = false;
    private currentSessionId: string | null = null;
    private statusData: Partial<WREStatus> = {};
    ...
    public setDashboard(dashboard: AgentDashboard) {
        this.dashboard = dashboard;
    }

    async runWRE(): Promise<WREResult> {
        try {
            ...
            if (wreResult.success) {
                ...
                this.statusData = {
                    ...
                    communicationActive: this.websocket?.readyState === WebSocket.OPEN,
                    contextSync: true
                };
    ...
    private async startWREProcess(cwd: string, directive: string): Promise<WREResult> {
        return new Promise((resolve) => {
            try {
                ...
                this.wreProcess = spawn(pythonCommand, args, {
                    cwd: cwd,
    ...
    async getStatus(): Promise<WREStatus> {
        return {
            ...
            communicationActive: this.websocket?.readyState === WebSocket.OPEN || false,
            contextSync: this.statusData.contextSync || false
        };
    }

    async stopWRE(): Promise<WREResult> {
        try {
            if (this.wreProcess && !this.wreProcess.killed) {
    ...
    async runWREOnPath(filePath: string): Promise<WREResult> {
    ...
    handleWorkspaceChange(event: vscode.WorkspaceFoldersChangeEvent): void {
        ...
        if (this.isActive && event.removed.length > 0) {
            ...
            vscode.window.showWarningMessage(
                'Workspace changed while WRE is active. Continue running?',
                'Yes', 'Stop WRE'
            ).then(selection => {
