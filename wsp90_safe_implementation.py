...
def test_safe_implementation():
    ...
    try:
        print("Hello World")
        print("Unicode: Hello 世界 [U+1F30D] (should work now)")
        print("[OK] Basic printing works")
    except Exception as e:
        print(f"[FAIL] Basic printing failed: {e}")
    ...
    try:
        ...
        if "This should be captured" in result:
            ...
            print(f"[FAIL] Stdout redirection failed: '{result}'")
    except Exception as e:
    ...
    try:
        ...
        if "Error message" in result:
            ...
            print(f"[FAIL] Stderr redirection failed: '{result}'")
    except Exception as e:
        print(f"[FAIL] Stderr redirection error: {e}")
    ...
    try:
        ...
        if "Test message" in content and "警告" in content:
            print("[OK] Logging works!")
        else:
            print(f"[FAIL] Logging failed: '{content}'")
    ...
    if sys.platform.startswith('win'):
        print("[U+1F5A5]️  Windows platform: UTF-8 enforcement is ACTIVE")
        print("   Using safe wrapper that preserves redirection functionality")
    else:
        print("[U+1F427] Non-Windows platform: UTF-8 enforcement is INACTIVE")
    ...
    print("\n[OK] CONCLUSION: Safe to use for mass WSP 90 enforcement!")
