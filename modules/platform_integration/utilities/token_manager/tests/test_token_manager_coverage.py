...
class TestTokenManagerCoverage(unittest.TestCase):
    ...
    @patch('modules.infrastructure.token_manager.src.token_manager.get_authenticated_service')
    @patch('modules.infrastructure.token_manager.src.token_manager.get_oauth_token_file')
    ...
    def test_check_token_health_default_current_token(self):
        ...
        with patch.object(self.manager, 'check_token_health', wraps=self.manager.check_token_health) as mock_check:
            with patch('modules.infrastructure.token_manager.src.token_manager.get_authenticated_service') as mock_get_service:
                with patch('modules.infrastructure.token_manager.src.token_manager.get_oauth_token_file') as mock_get_token_file:
                    mock_get_token_file.return_value = "test_token.json"
                    mock_service = MagicMock()
                    mock_service.channels().list().execute.return_value = {"items": [{}]}
                    mock_get_service.return_value = mock_service
