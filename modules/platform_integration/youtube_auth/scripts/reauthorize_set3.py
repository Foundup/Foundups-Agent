#!/usr/bin/env python3
"""
Re-authorization script for Set 3 credentials
Fixes expired/revoked tokens and 403 errors
"""
...
print('Re-authorizing Set 3 credentials...')
print('=' * 50)

# Check that client_secret3.json exists
client_secret_path = 'credentials/client_secret3.json'
if os.path.exists(client_secret_path):
    ...
    print('Please ensure you have the Google Cloud credentials for Set 3')
    sys.exit(1)
...
if os.path.exists(token_path):
    ...
    print('[OK] Cleared old token (was causing 403 errors)')
else:
...
print('[NOTE] The 403 error suggests the previous token was revoked')
print()
...
try:
    ...
    if response.get('items'):
        ...
        print()
        print('[OK] SUCCESS! Set 3 re-authorized!')
        print(f'[TV] Connected as: {channel}')
        print(f'[SAVE] Token saved to: {token_path}')
        print('[QUOTA] 10,000 quota units now available for Set 3!')
        print()
    else:
    ...
    if 'invalid_client' in str(e):
        print('The client_secret3.json file might be invalid.')
