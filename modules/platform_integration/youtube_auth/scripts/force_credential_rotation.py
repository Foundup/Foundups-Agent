...
def force_rotation_to_set_10():
    ...
    print("="*60)
    print("[REFRESH] FORCING CREDENTIAL ROTATION")
    print("="*60)
    ...
    if quota_file.exists():
        with open(quota_file, 'r', encoding="utf-8") as f:
            ...
            if "sets" in quota_data and "1" in quota_data["sets"]:
                ...
                print(f"[DATA] Set 1 Usage: {used}/10000 ({used/100:.1f}%)")
    ...
    if 1 not in data["exhausted_sets"]:
        ...
        print("[OK] Marked Set 1 as exhausted")
    else:
    ...
    try:
        ...
        print(f"[U+1F4C5] Next reset: {next_midnight_pt.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    except ImportError:
        ...
        data["next_reset"] = next_midnight_pt.isoformat()
        print(f"[U+1F4C5] Next reset (UTC-8): {next_midnight_pt.strftime('%Y-%m-%d %H:%M:%S')}")

    # Save the updated exhausted credentials
    with open(exhausted_file, 'w', encoding="utf-8") as f:
        json.dump(data, f, indent=2)

    print("[U+1F4BE] Saved exhausted credentials status")
    print("\n[TARGET] NEXT STEPS:")
