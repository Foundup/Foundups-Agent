...
# === END UTF-8 ENFORCEMENT ===
    ...
    def test_get_authenticated_service_quota_exceeded(self, mock_get_creds, mock_build):
        ...
        with patch('modules.platform_integration.youtube_auth.src.youtube_auth.os.path.exists', return_value=True) as mock_exists_inner:
            # Mock from_authorized_user_file to return appropriate creds
            with patch('modules.platform_integration.youtube_auth.src.youtube_auth.google.oauth2.credentials.Credentials.from_authorized_user_file') as mock_from_file_inner:
