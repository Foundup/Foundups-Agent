...
def check_bot_status():
    ...
    if not os.path.exists(log_path):
        print("[FAIL] No log file found - bot never ran")
        return
    ...
    if time_since.total_seconds() < 60:
        print("[OK] Bot is ACTIVE (log recently updated)")
    elif time_since.total_seconds() < 300:
        print("[U+26A0]️ Bot might be idle (no recent activity)")
    else:
        print("[FAIL] Bot is NOT running (log is old)")
