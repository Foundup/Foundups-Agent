...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/intensive_monitoring.log')  # WSP 85: Use logs/ directory
    ]
)
...
class IntensiveMonitor:
    ...
    async def check_all_systems(self):
        ...
        logger.info("=" * 80)
        logger.info("[SEARCH] INTENSIVE MONITORING SYSTEM CHECK")
        logger.info("=" * 80)
    ...
    async def check_consciousness_system(self):
        ...
        logger.info("\n[AI] CHECKING CONSCIOUSNESS SYSTEM (0102)")
        logger.info("-" * 40)
        ...
        try:
            ...
            for msg in test_messages:
                ...
                if "[U+270A][U+270B][U+1F590]️" in msg or "0102" in msg or "012" in msg:
                    logger.info(f"[OK] Consciousness trigger detected: {msg}")
                    self.components['consciousness'] = True
                    self.stats['consciousness_responses'] += 1
                    
        except Exception as e:
            logger.error(f"[FAIL] Consciousness system error: {e}")
            
    async def check_magadoom_system(self):
        """Test MAGADOOM timeout system"""
        logger.info("\n[GAME] CHECKING MAGADOOM WHACK SYSTEM")
        logger.info("-" * 40)
        ...
        try:
            ...
            for user in test_users:
                if "MAGA" in user.upper() or "TRUMP" in user.upper():
                    logger.info(f"[TARGET] MAGA detected: {user}")
                    
                    # Test timeout scoring
                    xp = tracker.calculate_xp_gain(user, is_maga=True)
                    logger.info(f"  [U+1F4B0] XP earned: {xp}")
                    
                    # Test announcements
                    if xp > 100:
                        logger.info(f"  [U+1F4E2] DUKE: 'Hail to the king, baby!'")
                    
                    self.components['magadoom'] = True
                    self.stats['maga_timeouts'] += 1
                    
            logger.info(f"[OK] MAGADOOM system active - {self.stats['maga_timeouts']} timeouts tracked")
                    
        except Exception as e:
            logger.error(f"[FAIL] MAGADOOM system error: {e}")
            
    async def check_command_system(self):
        """Test command processing"""
        logger.info("\n[LIGHTNING] CHECKING COMMAND SYSTEM")
        logger.info("-" * 40)
        ...
        try:
            ...
            for cmd in commands:
                logger.info(f"  [NOTE] Command available: {cmd}")
                self.stats['commands_executed'] += 1
                
            self.components['commands'] = True
            logger.info(f"[OK] Command system ready - {len(commands)} commands available")
            
        except Exception as e:
            logger.error(f"[FAIL] Command system error: {e}")
            
    async def check_grok_integration(self):
        """Test Grok 3 AI integration"""
        logger.info("\n[BOT] CHECKING GROK 3 INTEGRATION")
        logger.info("-" * 40)
        
        try:
            import os
            if os.getenv('XAI_API_KEY'):
                logger.info("[OK] Grok API key configured")
                logger.info("  Model: grok-3")
                logger.info("  Mode: 0102 consciousness responses")
                self.components['grok'] = True
            else:
                logger.warning("[U+26A0]️ Grok API key not found")
                
        except Exception as e:
            logger.error(f"[FAIL] Grok integration error: {e}")
            
    async def check_banter_engine(self):
        """Test banter engine responses"""
        logger.info("\n[U+1F4AC] CHECKING BANTER ENGINE")
        logger.info("-" * 40)
        ...
        try:
            ...
            logger.info(f"[OK] Banter engine loaded with {len(themes)} themes:")
    ...
    async def check_throttle_system(self):
        ...
        try:
            ...
            logger.info("[OK] Intelligent throttling active (5s to 30min)")
            
        except Exception as e:
            logger.error(f"[FAIL] Throttle system error: {e}")
            
    async def check_trigger_system(self):
        """Test trigger mechanism"""
        logger.info("\n[ALERT] CHECKING TRIGGER SYSTEM")
        logger.info("-" * 40)
        ...
        try:
            ...
            if Path("stream_trigger.txt").exists():
                logger.info("[OK] Trigger file exists")
                if trigger.check_trigger():
                    logger.info("  [ALERT] Trigger is ACTIVE!")
                else:
                    logger.info("  ⏸️ Trigger is idle")
            else:
                logger.info("  [NOTE] Trigger file not found (create with: echo TRIGGER > stream_trigger.txt)")
                
            self.components['trigger'] = True
            
        except Exception as e:
            logger.error(f"[FAIL] Trigger system error: {e}")
            
    async def check_owner_detection(self):
        """Test owner detection for Move2Japan"""
        logger.info("\n[U+1F451] CHECKING OWNER DETECTION")
        logger.info("-" * 40)
        
        try:
            # Check for Move2Japan as owner
            owner_channel = "UC-LSSlOZwpGIRIYihaz8zCw"
            logger.info(f"[OK] Owner channel configured: Move2Japan")
            ...
            logger.error(f"[FAIL] Owner detection error: {e}")
            
    def generate_report(self):
        """Generate monitoring report"""
        logger.info("\n" + "=" * 80)
        logger.info("[DATA] MONITORING REPORT")
        logger.info("=" * 80)
        ...
        logger.info("\n[U+1F3E5] OVERALL HEALTH:")
        logger.info(f"  {working}/{total} systems operational ({health:.0f}%)")
        
        if health == 100:
            logger.info("  [CELEBRATE] ALL SYSTEMS OPERATIONAL!")
        elif health >= 75:
            logger.info("  [U+26A0]️ MOSTLY OPERATIONAL - Some issues detected")
        else:
            logger.info("  [ALERT] CRITICAL - Multiple systems offline")
            
        logger.info("\n" + "=" * 80)
        logger.info("[IDEA] OWNER COMMANDS FOR Move2Japan:")
        logger.info("  • Type '[U+270A][U+270B][U+1F590]️' + message for 0102 consciousness response")
...
async def main():
    ...
    logger.info("\n[REFRESH] Monitoring will continue in background...")
    logger.info("Check 'logs/intensive_monitoring.log' for detailed logs")
