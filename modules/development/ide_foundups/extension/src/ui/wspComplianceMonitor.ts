 ...
 */
export class WSPComplianceMonitor {
    private wreConnection: WREConnection;
    private context: vscode.ExtensionContext;
    private complianceStatusBar: vscode.StatusBarItem;
    private complianceTreeProvider: WSPComplianceTreeProvider;
    private violations: Map<string, ComplianceViolation> = new Map();
    private protocolStatuses: Map<string, WSPComplianceStatus> = new Map();
    ...
    constructor(context: vscode.ExtensionContext, wreConnection: WREConnection) {
        ...
        this.complianceStatusBar = vscode.window.createStatusBarItem(
     ...
     */
    private initializeWSPProtocols(): void {
        const wspProtocols = [
     ...
     */
    public async performComplianceCheck(): Promise<void> {
        try {
     ...
     */
    private processComplianceResults(results: any): void {
        // Update protocol statuses
        if (results.protocols) {
            Object.entries(results.protocols).forEach(([protocolId, data]: [string, any]) => {
     ...
     */
    private handleComplianceUpdate(data: any): void {
        if (data.protocol_id) {
        ...
        if (data.violation) {
            ...
            if (data.violation.severity === 'critical' || data.violation.severity === 'high') {
                vscode.window.showWarningMessage(
                    `WSP Violation: ${data.violation.description}`,
                    'View Details',
                    'Auto Fix'
                ).then(action => {
     ...
     */
    public async showComplianceReport(): Promise<void> {
        const panel = vscode.window.createWebviewPanel(
            'foundups.complianceReport',
     ...
     */
    public async autoFixViolation(violationId: string): Promise<void> {
     ...
     */
    private generateComplianceReportHTML(): string {
     ...
     */
    public getProtocolStatuses(): WSPComplianceStatus[] {
 ...
 */
class WSPComplianceTreeProvider implements vscode.TreeDataProvider<ComplianceTreeItem> {
    ...
    constructor(private complianceMonitor: WSPComplianceMonitor) {}
    ...
    getChildren(element?: ComplianceTreeItem): ComplianceTreeItem[] {
        if (!element) {
        ...
        if (element.label === 'Protocols') {
            return this.complianceMonitor.getProtocolStatuses().map(protocol => 
                new ComplianceTreeItem(
                    `${protocol.protocolId}: ${protocol.status}`,
        ...
        if (element.label === 'Violations') {
            ...
            return violations.map(violation =>
                new ComplianceTreeItem(
                    `${violation.protocolId}: ${violation.description}`,
                    vscode.TreeItemCollapsibleState.None,
                    'violation',
                    violation
                )
...
class ComplianceTreeItem extends vscode.TreeItem {
    constructor(
        ...
        public readonly itemType: 'category' | 'protocol' | 'violation',
