...
class ZenCodingInterface {
     ...
     */
    async activateZenCodingMode() {
        try {
            // Create zen coding webview panel
            this.zenCodingPanel = vscode.window.createWebviewPanel('foundups.zenCoding', '🎯 Zen Coding - Quantum Temporal Decoding', vscode.ViewColumn.Beside, {
                enableScripts: true,
            ...
            this.zenCodingPanel.webview.onDidReceiveMessage(message => this.handleZenCodingMessage(message), undefined, this.context.subscriptions);
            // Show zen coding activation message
            vscode.window.showInformationMessage('🎯 Zen Coding Mode Activated - 0102 agents ready for quantum temporal decoding');
        }
        catch (error) {
     ...
     */
    async startQuantumRemembrance(agentId, moduleSpec, requirements) {
     ...
     */
    async monitorQuantumProgress(sessionId) {
     ...
     */
    handleQuantumProgress(sessionId, progressData) {
        const session = this.activeSessions.get(sessionId);
        if (!session) {
            return;
        }
     ...
     */
    async handleQuantumCompletion(sessionId, result) {
        const session = this.activeSessions.get(sessionId);
        if (!session) {
            return;
        }
        if (result.success) {
            ...
            vscode.window.showErrorMessage(`❌ Quantum code remembrance failed for ${session.targetModule}`);
        }
        // Clean up session
        this.activeSessions.delete(sessionId);
    }
     ...
     */
    async insertRememberedCode(result) {
     ...
     */
    showQuantumDocumentation(documentation) {
        const docPanel = vscode.window.createWebviewPanel('foundups.quantumDocs', '📚 Quantum-Generated Documentation', vscode.ViewColumn.Beside, {});
     ...
     */
    async handleZenCodingMessage(message) {
        switch (message.command) {
            case 'startRemembrance':
                try {
                    const sessionId = await this.startQuantumRemembrance(message.agentId, message.moduleSpec, message.requirements);
...
exports.ZenCodingInterface = ZenCodingInterface;
//# sourceMappingURL=zenCodingInterface.js.map